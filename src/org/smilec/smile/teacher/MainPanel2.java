...
public class MainPanel2 extends JPanel {
	...
	Font newButtonFont, zoomBtnFont, smallButtonFont;
	...
	void set_locale(String chosen_language) {
		...
		if (chosen_language.equals("Arabic")){
		...
		} else if(chosen_language.equals("Gujarati (India)")) {  // Shruti on Windows
			cur_locale = new Locale("hi_Gujarati");
			main.set_locale("hi_Gujarati");
			data_source.set_locale("hi_Gujarati");
			spinner1.setSelectedIndex(2); 
		} else if(chosen_language.equals("Tamil (India)")) {   // Latha on Windows
			cur_locale = new Locale("hi_Tamil");
			main.set_locale("hi_Tamil");
			data_source.set_locale("hi_Tamil");
			spinner1.setSelectedIndex(3); 
		} else if(chosen_language.equals("Hindi (India)")) {
			cur_locale = new Locale("hi_HI");
			main.set_locale("hi_Ujjwal");
			data_source.set_locale("hi_Ujjwal");
			...
			spinner1.setSelectedIndex(6); 
		} else if (chosen_language.equals("Swahili")){
			cur_locale = new Locale("sw_SW");
			main.set_locale("sw_SW");
			data_source.set_locale("sw_SW");
			spinner1.setSelectedIndex(7); 
		} else if (chosen_language.equals("Thai")){
			cur_locale = new Locale("th_TH");
			main.set_locale("th_TH");
			data_source.set_locale("th_TH");
			spinner1.setSelectedIndex(8); 
		} */
		
		int idx = langList.indexOf(chosen_language);
		if(idx==-1) idx=1;  // English
		String lang_dir = langDir.get(idx);
		//System.out.println("lang_dir:" + lang_dir + ":\n" );
		cur_locale = new Locale(lang_dir);
		main.set_locale(lang_dir);
		data_source.set_locale(lang_dir);
		spinner1.setSelectedIndex(idx); 
		...
		if (chosen_language.equals("Arabic")||chosen_language.equals("Urdu")){
			qv_pane.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			question_pane.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			sc_pane.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			msgHighRank.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
		} 
		else
		{
			qv_pane.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
			question_pane.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
			sc_pane.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
			msgHighRank.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
		}
	}
	...
	class ControlPanel extends JPanel {
		...
		JPanel create_set_panel(int size_X, int size_Y) {
		    ...
		    langList= Arrays.asList(language_list);
		    langDir = Arrays.asList(conf.getProperty("LANG_DIR").split(","));
			...
			spinner1.setBounds(gridX(0), gridY(8.6), x_size+50, y_size); // 
			setImportEnabled(false);
		...
		JPanel create_flow_panel(int size_X, int size_Y) {
			...
			smallButtonFont=new Font(btnConnect.getFont().getName(), Font.BOLD, 9); 	
	...
	public void rewrite_btn_panel()
	{
		...
		if(current_language.equals("Tamil (India)")||current_language.equals("Malayalam (India)"))
	    {
	    	btnRetakeQuiz.setFont(smallButtonFont);
	    	btnStartSession.setFont(smallButtonFont);
	    }
	    else
	    {
	    	btnRetakeQuiz.setFont(newButtonFont);
